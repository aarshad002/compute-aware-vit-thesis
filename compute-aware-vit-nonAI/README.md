...
This folder contains the first study of the thesis *Compute-Aware Vision Transformers: Adaptive Inference and AI-Assisted Research Development*, the manually developed research implementation on **compute-efficient Vision Transformers**. The central question is: can a ViT automatically decide, per image, how many tokens it actually needs — and skip the rest?
...
**Training strategies explored.** Ten controller runs were carried out across two training
paradigms. Every one suffered **budget collapse** — the controller converged to predicting a
single budget for all images. The per-run evidence (verified against the recorded
per-epoch budget counts) is in [`docs/10`](docs/10_learned_budget_controller.md); the
summary is below.

*Paradigm A — Gumbel-softmax (end-to-end, differentiable routing).* Straight-through
Gumbel-softmax makes the discrete budget choice differentiable; the loss is
cross-entropy + a budget penalty `mean(expected_keep_ratio × (1 − confidence))` that is
meant to penalise cheap budgets on hard (low-confidence) images.

- `gumbel_v1` — 3 budgets, `batch_size=1`, backbone trainable. Collapsed to the **25%
  budget for all 10,000 val images from epoch 1**; the rising accuracy (to 61.16%) is
  merely the backbone learning to classify under a fixed 25% budget, not routing.
- `gumbel_v2` — loads and **freezes** the fixed-50% backbone, trains only the controller
  with a stronger penalty (`loss_weight=0.1`, `lr=1e-3`). Every epoch still routes all
  images to 25% (val acc frozen at 74.14%).
- `e2e_v2` — adds a dense **knowledge-distillation teacher** (`distillation_weight=0.5`,
  `T=4`) and a 50/50 val split. Still collapses to 25% every epoch (val acc 77.74%).

*Paradigm B — supervised (oracle budget labels).* Oracle labels are generated by running
all four fixed-budget models and labelling each image with the *smallest* budget that
classifies it correctly, then training the controller with cross-entropy. Note: in this
path the reported metric is **budget-prediction accuracy (4-way)**, not image accuracy —
and because the val label distribution is 75.8% "25%", predicting "always 25%" *scores
exactly 0.7583*, which is the collapse signature, not a real result.

- `supervised_v1` / `supervised_v3` — class-weighted CE. Both predict 25% for all val
  images; the 75.83% is exactly the majority-class rate.
- `ce_v1` — plain CE. **Unstable**: flips which single class it favours across epochs
  (all-25% → all-50% → all-dense), val accuracy oscillating 5–76%.
- `conf_v1` — confidence-derived labels: same majority-class regime (75.62%).
- `focal_v1` — focal loss (γ=2): collapse moved onto the **75% class** instead, scoring a
  catastrophic 3.90% (near that class's val frequency).
- `split_v1` — separate ctrl-train/ctrl-val label split: single-class dominated (65.50%).
- **balanced-label runs** — trained on a class-balanced subset (~25% per budget) so
  majority-class shortcutting is impossible: budget-prediction accuracy is only **~29%**,
  barely above the 25% random-guess floor.

| Run | Paradigm | Metric | Value | Behaviour |
|---|---|---|---|---|
| gumbel_v1 | Gumbel, bs=1 | image acc | 61.16% | collapse → 25% |
| gumbel_v2 | Gumbel, frozen backbone | image acc | 74.14% | collapse → 25% |
| e2e_v2 | Gumbel + distillation | image acc | 77.74% | collapse → 25% |
...
| ce_v1 | plain CE | budget-pred acc | 75.72% | unstable, one class per epoch |
| conf_v1 | CE (confidence labels) | budget-pred acc | 75.62% | = majority baseline |
| focal_v1 | focal loss γ=2 | budget-pred acc | 3.90% | collapse → 75% |
| split_v1 | CE, split labels | budget-pred acc | 65.50% | single-class dominated |
| balanced | CE, balanced subset | budget-pred acc | ~29.0% | ≈ random (25%) |

**Root causes (three, all evidenced).** (1) The oracle **labels are severely skewed** —
train labels are 96.8% "25% suffices", so CE collapses to the majority class, and
re-weighting or focal loss only *moves* the collapse to a different single class. (2) The
**difficulty signal at layer 6 is too weak**: on the balanced labels, where the majority
shortcut is impossible, budget-prediction accuracy is ~29% versus a 25% chance floor — the
12 features simply do not separate "needs 25%" from "needs 75%". (3) **Discrete routing is
hard to optimise end-to-end** — Gumbel-softmax collapsed within one epoch in every setting,
even with a frozen backbone, a stronger penalty, and distillation. Budget collapse here is
therefore a fundamental signal/label problem, not an untuned hyperparameter, and it is what
motivates the zero-parameter rule controller below.
