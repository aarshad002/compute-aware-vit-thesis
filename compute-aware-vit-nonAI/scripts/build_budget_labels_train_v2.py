...
def main():
    ...
    for budget_idx, budget in enumerate(budgets):
        ...
        # load the corresponding fine-tuned checkpoint
        checkpoint_map = {
        ...
        ckpt = checkpoint_map[budget]
        state = torch.load(ROOT / ckpt, map_location=device)
    ...
    # remaining samples that no budget got right → assign label 3
    total = len(train_loader.dataset)
    for idx in range(total):
        if idx not in budget_labels:
            budget_labels[idx] = 3

    output_path = ROOT / "data" / "budget_labels_train_v2.json"
